...
<form method="POST" action="{{ route('kelas.update', $kelas->id) }}">
    ...
    <div class="py-6 first:pt-0 last:pb-0 border-t first:border-transparent border-gray-200">
        <label for="nama_kelas" class="inline-block text-sm font-medium">
            Nama
        </label>
        <div class="mt-1 mb-2 space-y-3">
            <input id="nama_kelas" name="nama_kelas" type="text" value="{{ old('nama_kelas', $kelas->nama_kelas) }}" class="py-1.5 sm:py-2 px-3 block w-full border border-gray-200 shadow-2xs sm:text-sm rounded-lg focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" placeholder="Masukkan nama kelas" required>
            @error('nama_kelas')
        ...
        <div class="mt-1 space-y-3">
            ...
            <option value="{{ old('id_guru') ? '' : 'selected' }}" disabled selected>Guru</option>
            @foreach ($guru as $item)
              <option value="{{ $item->id }}" {{ old('id_guru', $kelas->id_guru) == $item->id ? 'selected' : '' }}>{{ $item->nama }}</option>
            @endforeach
            </select>
            @error('id_guru')
